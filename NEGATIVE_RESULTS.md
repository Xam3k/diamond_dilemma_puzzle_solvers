# Diamond Dilemma — What We Tried That Didn't Work (and How We Decided)

*Companion to `SOLVERS.md`. Every abandoned approach, the measurement or
argument that killed it, and — importantly — an honest note on whether the
...
a time** — using these yardsticks. Knowing the yardstick matters, because a
...
- **Why (diagnosis):** our encoding, not necessarily SAT itself; the model was
...
- **Still holds?** Mostly. A 64–128-worker cloud attempt is qualitatively
  different and untried — but we deprioritized it because the puzzle is
...
- **Hypothesis (user's edge-imbalance idea):** better rarity measure in the
...
- **Still holds?** This is a judgment call, not a measurement. A successor
  who believes UNSAT should run massive parallel CP-SAT — nothing we did
  refutes that path; we only priced it (uncertain odds, five-figure bill).
...
## 6. Measurement fallacies we caught (check yourself against these)
