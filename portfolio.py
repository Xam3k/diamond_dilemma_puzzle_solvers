"""Run a portfolio of solver2.exe processes (different seeds explore different
...
for s in range(1, nproc + 1):
    lf = open(f"s2_seed{s}.log", "w")
    p = subprocess.Popen(["./solver2.exe", "instance_gold.txt", node_limit, str(s)],
                         stdout=lf, stderr=lf)
    procs.append(p); logs.append(f"s2_seed{s}.log")
